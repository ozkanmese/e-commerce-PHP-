			...
			if($GelenOdemeTuruSecimi == "Banka Havalesi"){
				...
				if($SepetSayisi > 0){
					foreach($SepetUrunleri as $SepetSatirlari){
						...
						$SiparisEkle = $VeritabaniBaglantisi->prepare("INSERT INTO siparisler (UyeId, SiparisNumarasi, UrunId, UrunTuru, UrunAdi, UrunFiyati, KdvOrani, UrunAdedi, ToplamUrunFiyati, KargoFirmasiSecimi, KargoUcreti, UrunResmiBir, VaryantSecimi, AdresAdiSoyadi, AdresDetay, AdresTelefon, OdemeSecimi, TaksitSecimi, SiparisTarihi, SiparisIpAdresi) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
						$SiparisEkle->execute([$SepettekiUyeId,$SepetSepetNumarasi,$SepettekiUrunId,$UrununTuru,$UrununAdi,$UrunFiyatiHesapla,$UrununKdvOrani,$SepettekiUrunAdedi,$UrununToplamFiyati,$KargonunAdi,$UrununToplamKargoFiyati,$UrununResmi,$VaryantAdi,$AdresAdiSoyadi,$AdresToparla,$AdresTelefonNumarasi,$GelenOdemeTuruSecimi,0,$ZamanDamgasi,$IPAdresi]);
						...
						if($EklemeKontrol > 0){
							...
							header("Location:index.php?SK=115");
							exit();
						}
					}
					...
					header("Location:index.php?SK=114");
					exit();
				}else{
				...
				if($GelenTaksitSecimi != ""){
					...
					if($SepetKontrol > 0){
						header("Location:index.php?SK=116");
						exit();
					}else{
						header("Location:index.php");
						exit();
