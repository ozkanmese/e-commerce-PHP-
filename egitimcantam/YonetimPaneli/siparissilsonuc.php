			...
			foreach($SiparisKayitlari as $Satirlar){
				...
				if($SilmeKontrol>0){
					...
					if($UrunGuncellemeKontrol>0){
						$VaryantGuncellemeSorgusu	=	$VeritabaniBaglantisi->prepare("UPDATE urunvaryantlari SET StokAdedi=StokAdedi+? WHERE UrunId = ? LIMIT 1");
						$VaryantGuncellemeSorgusu->execute([$SiparistekiUrununAdedi,  $SiparistekiUrununIDsi]);
