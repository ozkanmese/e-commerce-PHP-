...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenUrunMenusu!="") and ($GelenUrunAdi!="") and ($GelenUrunFiyati!="") and ($GelenParaBirimi!="") and ($GelenKdvOrani!="") and ($GelenKargoUcreti!="") and ($GelenUrunAciklamasi!="") and ($GelenVaryantAdi1!="") and ($GelenStokAdedi1!="")){
		...
		if($UrunKontrol>0){
			...
			if($MenuTuruKontrol>0){
				$UrunGuncellemeSorgusu		=	$VeritabaniBaglantisi->prepare("UPDATE urunler SET MenuId = ?, UrunAdi = ?, UrunFiyati = ?, ParaBirimi = ?, KdvOrani = ?, UrunAciklamasi = ?, KargoUcreti = ? WHERE id = ? LIMIT 1");
				$UrunGuncellemeSorgusu->execute([$GelenUrunMenusu, $GelenUrunAdi, $GelenUrunFiyati, $GelenParaBirimi, $GelenKdvOrani, $GelenUrunAciklamasi,  $GelenKargoUcreti, $GelenID]);
