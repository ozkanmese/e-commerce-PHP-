...
if(isset($_SESSION["Yonetici"])){
	...
	if(($GelenUrunMenusu!="") and ($GelenUrunAdi!="") and ($GelenUrunFiyati!="") and ($GelenParaBirimi!="") and ($GelenKdvOrani!="") and ($GelenKargoUcreti!="") and ($GelenUrunAciklamasi!="") and ($GelenVaryantAdi1!="") and ($GelenStokAdedi1!="") and ($GelenResim1["name"]!="") and ($GelenResim1["type"]!="") and ($GelenResim1["tmp_name"]!="") and ($GelenResim1["error"]==0) and ($GelenResim1["size"]>0)){
        ...
        if($MenuTuruKaydi["UrunTuru"] == "Ders Kitabı"){
            $ResimKlasoru = "/Urunler/DersKitabi/";
        }elseif($MenuTuruKaydi["UrunTuru"] == "Okuma Kitabı"){
            $ResimKlasoru = "/Urunler/OkumaKitaplari/";
        }elseif($MenuTuruKaydi["UrunTuru"] == "Soru Bankası"){
            $ResimKlasoru = "/Urunler/SoruBankasi/";
        }elseif($MenuTuruKaydi["UrunTuru"] == "Dergi"){
            $ResimKlasoru = "/Urunler/Dergiler";
        }elseif($MenuTuruKaydi["UrunTuru"] == "Üniversite Kitabı"){
            $ResimKlasoru = "/Urunler/UniversiteKitaplari/";
        }
        ...
        if($MenuTuruKontrol>0){
			...
			$UrunEklemeSorgusu		=	$VeritabaniBaglantisi->prepare("INSERT INTO urunler (MenuId, UrunTuru, UrunAdi, UrunFiyati, ParaBirimi, KdvOrani, UrunAciklamasi, UrunResmiBir, KargoUcreti, Durumu) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			$UrunEklemeSorgusu->execute([$GelenUrunMenusu, $MenuTuruKaydi["UrunTuru"], $GelenUrunAdi, $GelenUrunFiyati, $GelenParaBirimi, $GelenKdvOrani, $GelenUrunAciklamasi, $BirinciResimIcinYeniDosyaAdi,  $GelenKargoUcreti, 1]);
